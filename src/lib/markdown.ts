...
md.renderer.rules.fence = (tokens, idx) => {
  ...
  return `<div class="code"><div class="code-head"><span>${md.utils.escapeHtml(lang || 'text')}</span><button type="button" class="code-copy" data-copy>Copy</button><button type="button" class="code-copy" data-download="${md.utils.escapeHtml(lang || 'txt')}">Download</button></div><pre><code${lang ? ` class="language-${md.utils.escapeHtml(lang)}"` : ''}>${code}</code></pre></div>`
}
