...
- **Stop-loss** per chat request, per stage and per circuit. Requests that cannot fit are refused, `max_tokens` is capped to what is left, streams abort when they cross the line, and circuits can *squeeze* (drop context, trim, go fast) before stopping. Runs stopped by a token stop-loss, step limit, or the Stop button can be **resumed** from a saved local checkpoint, even after reloading the app. Completed stages, exact saved model messages, shared memory, loop counters, feedback and review comments are retained. The Resume dialog lets you add tokens, adjust the next stage’s cap and choose another step allowance. Interrupted attempts stay in the transcript and usage totals; an interrupted model request restarts with its saved context and partial work. Completed media items are reused. An interrupted external action/tool may already have taken effect, so retrying it requires acknowledgement. Older runs without checkpoints can recover outputs and memory, but not missing original prompts.
- **Resume costs.** Resending context consumes input tokens, new output is billed normally, and a retried request may add to the charges for its interrupted attempt. Provider cache discounts are not guaranteed. There is no cache-based expiry for Resume: saved context remains usable while the run is stored locally. Clearing site data removes it.
